const setUpSellPage = () => {
  ...
  $('.buy-plus').on('click', function () {
    ...
    if (newValue <= cartData[productName].inventoryCount){
      if (!data.infinite){
        if (newValue <= cartData[productName].max - cartData[productName].dbCount) {
          $(`.product-price-input[data-product="${productName}"]`).val(newValue)
          cartData[productName].count = newValue
          $('.details').html(genCartSell(cartData))
          setSellButton()
  ...
  $('.product-price-input').on('input', function(){
    ...
    if($(this).val() > cartData[productName].inventoryCount) {
      ...
      if (data.infinite || cartData[productName].inventoryCount < (cartData[productName].max - cartData[productName].dbCount)){
    ...
    if(!data.infinite && $(this).val() > cartData[productName].max - cartData[productName].dbCount) {
      $(this).val(cartData[productName].max - cartData[productName].dbCount)
    }
